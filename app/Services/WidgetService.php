...
class WidgetService extends BaseService
{
    ...
    private function getObjectsWithRelationships(string $tableName, array $modelIds, string $model, int $language): Collection
    {
        ...
        // Xử lý model name để lấy tên class đơn giản
        $modelName = class_basename($model);
        $languageTable = Str::snake($modelName) . '_language';
        $catalogueTable = Str::snake($modelName) . '_catalogues';
        $catalogueLanguageTable = Str::snake($modelName) . '_catalogue_language';
        $pivotTable = Str::snake($modelName) . '_catalogue_' . Str::snake($modelName);
        $modelIdList = implode(',', array_map('intval', $modelIds));
        ...
        $modelField = Str::snake($modelName) . '_id';
        $catalogueField = Str::snake($modelName) . '_catalogue_id';
        ...
        $sql .= "
            FROM {$tableName} o
            LEFT JOIN {$languageTable} ol ON o.id = ol.{$modelField} AND ol.language_id = ?
            LEFT JOIN {$pivotTable} pc ON o.id = pc.{$modelField}
            LEFT JOIN {$catalogueTable} c ON pc.{$catalogueField} = c.id
            LEFT JOIN {$catalogueLanguageTable} cl ON c.id = cl.{$catalogueField} AND cl.language_id = ?
        ";
        ...
        return collect(DB::select($sql, [$language, $language]))->map(function ($item) use ($modelName) {
            ...
            if ($item->catalogue_ids) {
                $catalogueIds = explode(',', $item->catalogue_ids);
                $catalogueNames = explode(',', $item->catalogue_names);
                $relationName = Str::snake($modelName) . '_catalogues';
    ...
    {
        // Lấy tên class cuối cùng (sau dấu \ cuối cùng)
    ...
    {
        // objectModel đã là tên class đơn giản (post, product) từ getObjectModel
